...
static void wavelengthToRGB(double lam_nm, double& r, double& g, double& b) {
    r = g = b = 0.0;
    if      (lam_nm < 440.0) { r = -(lam_nm - 440.0) / 60.0; b = 1.0; }
    else if (lam_nm < 490.0) { g =  (lam_nm - 440.0) / 50.0; b = 1.0; }
    else if (lam_nm < 510.0) { g = 1.0; b = -(lam_nm - 510.0) / 20.0; }
    else if (lam_nm < 580.0) { r =  (lam_nm - 510.0) / 70.0; g = 1.0; }
    else if (lam_nm < 645.0) { r = 1.0; g = -(lam_nm - 645.0) / 65.0; }
    else                     { r = 1.0; }
...
static std::vector<double> simulateWaves(double lam_nm, double slit_w_um,
                                           ...
                                           int nx, int ny, double Z_um, double Y_um,
                                           double time_phase) {
    ...
    int src_per_slit = std::max(1, (int)(w / (lam / 4.0)));
    if (src_per_slit > 64) src_per_slit = 64;

    std::vector<double> sources;
    sources.reserve(n_slits * src_per_slit);
    for (int s = 0; s < n_slits; ++s) {
        const double centre = (n_slits == 1) ? 0.0 : (s - 0.5 * (n_slits - 1)) * d;
        for (int i = 0; i < src_per_slit; ++i) {
            const double t = (i + 0.5) / src_per_slit - 0.5;
    ...
    dispatch_apply(ny, dispatch_get_global_queue(QOS_CLASS_USER_INTERACTIVE, 0), ^(size_t py) {
        ...
        for (int px = 0; px < nx; ++px) {
            ...
            double min_r = lam / 10.0;
            for (double ys : sources) {
                const double dy = y - ys;
                double r = std::sqrt(z * z + dy * dy);
                if (r < min_r) r = min_r;
                const double phi = k * r - time_phase;
                E += std::cos(phi) / std::sqrt(r);
            }
...
static NSImage* renderFrame(double lam_nm, double slit_w_um, double slit_sep_um,
                          int n_slits, int width_px, int height_px, double time_phase) {
    double Z_um = 20.0;
    double Y_um = 10.0;
    auto field = simulateWaves(lam_nm, slit_w_um, slit_sep_um, n_slits,
                                 width_px, height_px, Z_um, Y_um, time_phase);
    
    double cr, cg, cb; wavelengthToRGB(lam_nm, cr, cg, cb);
    
    double expected_max = (n_slits * std::max(1.0, slit_w_um / (lam_nm * 1e-3 / 4.0))) / std::sqrt(Z_um * 0.1);
    if (expected_max <= 0) expected_max = 1.0;
    // Scale factor to make it look bright enough
    double scale = 1.5 / expected_max;

    NSBitmapImageRep* rep = [[NSBitmapImageRep alloc]
        initWithBitmapDataPlanes:NULL pixelsWide:width_px pixelsHigh:height_px
        bitsPerSample:8 samplesPerPixel:3 hasAlpha:NO isPlanar:NO
        colorSpaceName:NSDeviceRGBColorSpace bytesPerRow:width_px * 3 bitsPerPixel:24];
        
    unsigned char* buf = [rep bitmapData];
    double* field_ptr = field.data();
    
    dispatch_apply(height_px, dispatch_get_global_queue(QOS_CLASS_USER_INTERACTIVE, 0), ^(size_t py) {
        unsigned char* row = buf + py * (width_px * 3);
        for (int px = 0; px < width_px; ++px) {
            double E = field_ptr[py * width_px + px];
            double val = E * scale;
            
            // Map positive field to brightness of the wavelength color
            double v = val > 0 ? val : 0.0;
            if (v > 1.0) v = 1.0;
            
            // Add a slight background glow for the squared intensity so we can see the interference fringes even in the dark parts
            double intensity = E * E * scale * scale * 0.1;
            if (intensity > 1.0) intensity = 1.0;
            
            v = v * 0.8 + intensity * 0.2;
            if (v > 1.0) v = 1.0;
...
- (NSSlider*)createSlider:(CGFloat)y minValue:(double)mn maxValue:(double)mx
              initial:(double)init step:(double)step {
    NSSlider* s = [[NSSlider alloc] initWithFrame:NSMakeRect(140, y, 740, 22)];
    s.minValue = mn;
    s.maxValue = mx;
    s.doubleValue = init;
    if (step > 0) { s.allowsTickMarkValuesOnly = YES;
                    s.numberOfTickMarks = (int)((mx - mn) / step) + 1; }
    s.target = self; s.action = @selector(onSliderChanged:);
    s.continuous = YES;
...
- (NSTextField*)createLabel:(CGFloat)y text:(NSString*)t width:(CGFloat)w {
    NSTextField* f = [[NSTextField alloc] initWithFrame:NSMakeRect(20, y - 2, w, 22)];
    f.stringValue = t; f.bezeled = NO; f.editable = NO; f.selectable = NO;
    f.drawsBackground = NO; f.font = [NSFont systemFontOfSize:13 weight:NSFontWeightMedium];
    [self.window.contentView addSubview:f];
    return f;
}

- (NSTextField*)createReadout:(CGFloat)y {
    NSTextField* f = [[NSTextField alloc] initWithFrame:NSMakeRect(890, y - 2, 90, 22)];
...
- (void)applicationDidFinishLaunching:(NSNotification*)note {
    ...
    self.imageView = [[NSImageView alloc] initWithFrame:NSMakeRect(20, 230, 960, 480)];
    ...
    [self createLabel:170 text:@"λ (nm)" width:120];
    self.lamSlider = [self createSlider:170 minValue:380.0 maxValue:780.0 initial:500.0 step:0];
    self.lamLabel  = [self createReadout:170];

    [self createLabel:135 text:@"Width (μm)" width:120];
    self.wSlider   = [self createSlider:135 minValue:0.1   maxValue:5.0 initial:0.5   step:0];
    self.wLabel    = [self createReadout:135];

    [self createLabel:100 text:@"Slit Distance (μm)" width:130];
    self.dSlider   = [self createSlider:100 minValue:1.0  maxValue:10.0 initial:3.0 step:0];
    self.dLabel    = [self createReadout:100];

    [self createLabel: 65 text:@"Slits" width:120];
    self.nSlider   = [self createSlider: 65 minValue:1.0   maxValue:5.0   initial:2.0   step:1];
    self.nLabel    = [self createReadout: 65];
    ...
    self.statusLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(120, 24, 860, 22)];
    self.statusLabel.bezeled = NO; self.statusLabel.editable = NO;
    self.statusLabel.selectable = NO; self.statusLabel.drawsBackground = NO;
...
- (void)onTogglePlay:(id)sender {
    if (self.playButton.state == NSControlStateValueOn) {
        ...
        self.timer = [NSTimer scheduledTimerWithTimeInterval:1.0/60.0 target:self selector:@selector(onTimerTick:) userInfo:nil repeats:YES];
...
- (void)onSliderChanged:(id)sender {
    ...
    self.imageView.image = renderFrame(lam, w, d, n, 960, 480, self.time_phase);
    ...
    if (n >= 2) {
        self.statusLabel.stringValue =
            [NSString stringWithFormat:
             @"Microscopic 2D Wave Propagation: Z = %.1f μm, Y = %.1f μm. Multi-slit interference.", Z_um, Y_um];
    } else {
        self.statusLabel.stringValue =
            [NSString stringWithFormat:
             @"Microscopic 2D Wave Propagation: Z = %.1f μm, Y = %.1f μm. Single slit diffraction.", Z_um, Y_um];
    }
}
